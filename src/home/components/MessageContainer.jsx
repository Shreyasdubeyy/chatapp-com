...
const MessageContainer = ({ onBackUser }) => {
    ...
    return (
        ...
        {selectedConversation === null ? (
          ...
          <>
            <div className='flex justify-between gap-1 bg-purple-300 md:px-2 rounded-lg h-10 md:h-12'>
              ...
              {!loading && messages?.length > 0 && messages?.map((message) => (
                <div className='text-white' key={message?._id} ref={lastMessageRef}>
                  <div className={`chat ${message.senderId === authUser._id ? 'chat-end' : 'chat-start'}`}>
                    ...
                    <div className={`chat-bubble ${message.senderId === authUser._id ? 'bg-purple-500 text-white' : ''

                    }`}>
                      {message?.message}
                    </div>
                    <div className="chat-footer text-[10px] opacity-90">
            ...
            <div className='w-full rounded-full flex items-center bg-white'>
              ...
              <button type='submit'>
                ...
                <IoSend size={25}
                className='text-emerald-700 cursor-pointer rounded-full bg-gray-800 w-10 h-auto p-1'/>
                }
              </button>
            </div> 
            </form>
